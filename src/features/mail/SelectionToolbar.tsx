...
export function SelectionToolbar({
  ...
  mailboxId: string
  mailboxes: Mailbox[]
  /** How many rows the list currently has, for the "select all" offer. */
  loadedCount: number
}) {
  ...
  return (
    <div className="flex items-center gap-1 px-2.5 pt-2.5 pb-1.5" data-testid="selection-toolbar">
      <button
        type="button"
        title={t('bulk.clear')}
      ...
      {count >= loadedCount && loadedCount > 0 && (
        <button
          type="button"
          onClick={() => void selectWholeFolder()}
          disabled={busy}
          className="ml-1 text-xs whitespace-nowrap text-accent hover:underline disabled:opacity-40"
        >
          {t('bulk.selectAll')}
        </button>
      )}
      ...
      <span className="ml-auto flex items-center">
        ...
        <ToolbarButton
          ...
          disabled={busy}
          onClick={() =>
            void run(() => bulkSetKeyword(accountId, selection, '$flagged', true), t('bulk.marked'))
          }
        />
